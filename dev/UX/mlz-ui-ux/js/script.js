...
//simple marquee news ticker 
if ($.isFunction($.fn.SimpleMarquee)) {
	$(function (){		
		let options = {
			autostart: true,
			property: 'value',
...
// add popup upload photo
	$('.edit-prof').on('click', function () {
		...
		$('.popup-closed').on('click', function () {
		$('.popup-wraper, .popup-wraper1, .popup-add-card').removeClass('active');
	});	
...
// new reward button
	$('.new-reward-btn').on('click', function () {
		$(this).toggleClass('active');
		$('.new-chp-rwd').toggleClass('active');
	});	
...
//-- page caro
	...
	// become partner carousel
		$('.become-partner').owlCarousel({
			loop:true,
			margin:28,
			smartSpeed: 1000,
			responsiveClass:true,
...
// date and time picker 
	if ($.isFunction($.fn.datetimepicker)) {		
		...
		$('#datetimepicker').datetimepicker({value:'Select Date & Time'});
	}	
